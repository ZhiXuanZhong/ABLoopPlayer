...
// Add these with the other var declarations near the top of the file (around line 19)
var loopPauseMs = 0; // pause between loops in milliseconds
var loopPauseTimeout = null; // timeout handle for loop pauses
...
$(document).ready(function () {
  ...
  // Add loop pause input handler
  $("#loopPause").on("change", function () {
    loopPauseMs = Math.min(Math.max(this.value, 0), 5000);
    this.value = loopPauseMs;
  });
...
var onScrubTimerUpdate = function () {
  ...
  if ((tMedia < timeA && !intro.checked) || tMedia >= timeB) {
    ...
    // Handle loop pause
    if (tMedia >= timeB && loopPauseMs > 0) {
      pauseVideo();
      if (loopPauseTimeout) clearTimeout(loopPauseTimeout);
      loopPauseTimeout = setTimeout(() => {
        setCurrentTime(timeA);
        playVideo();
      }, loopPauseMs);
      return;
...
var cancelABLoop = function () {
  ...
  // Clear loop pause timeout
  if (loopPauseTimeout) {
    clearTimeout(loopPauseTimeout);
    loopPauseTimeout = null;
  }
};
    ...
    if (ids.length) {
      ids.forEach((id) => {
        if (data["ab." + id] && JSON.parse(data["ab." + id]).length) {
          ...
          bmks = bmks.map((bmk) => {
            if (bmk.note)
              return { ta: Number(bmk.ta), tb: Number(bmk.tb), note: bmk.note };
...
var onBmkSelect = function (idx) {
  ...
  // Clear any existing loop pause timeout
  if (loopPauseTimeout) {
    clearTimeout(loopPauseTimeout);
    loopPauseTimeout = null;
  }
